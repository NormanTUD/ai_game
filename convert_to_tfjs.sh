...
if command -v yolo &> /dev/null; then
    ...
    chmod -R a+rw /usr/local/lib/python3.11/site-packages/onnx2tf/ 2>/dev/null || true

    cd $output_dir_host

    copied_model_file="$output_dir_host/$uuid.pt"

    if [[ -e $model_file ]]; then
	#echo "Copying $model_file to $copied_model_file"
	cp "$model_file" "$copied_model_file"
    fi

    #echo "yolo export model=\"$copied_model_file\" format=tfjs"
    yolo export model="$copied_model_file" format=tfjs

    web_model_dir="$output_dir_host/${uuid}_web_model/"

    if [[ ! -d $web_model_dir ]]; then
	    echo "Cannot find files under >$web_model_dir< (web_model_dir: $web_model_dir, output_dir_host: $output_dir_host, output_dir_host: $output_dir_host)"
    fi
else
    echo "Local YOLO not found, using Docker..."
    dockerfile_path="$output_dir_host/Dockerfile"
...
EOF
    ...
    docker build -t "$image_tag" -f "$dockerfile_path" "$output_dir_host" || { echo "Docker build failed"; exit 1; }
